...
public class UserSessionActorSpecs : TestKit
{
    ...
    public async Task UserSessionActorShouldJoinAndLeaveChatroomHappyPath()
    {
        ...
        IActorRef chatRoomManager = await ActorRegistry.GetAsync<RoomManagerActor>();
        ...
        IActorRef userSessionManager = await ActorRegistry.GetAsync<UserSessionManager>();

        // act
        
        // populate user and join chatroom
        foreach (var c in commandsForUser)
        {
            userSessionManager.Tell(c, TestActor);
        }
